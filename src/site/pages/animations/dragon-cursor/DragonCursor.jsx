...
const DragonCursor = () => {
  usePageMeta({
    title: "Dragon Cursor Effect",
    description:
      "Animated SVG dragon that follows, swirls and morphs around your cursor.",
    keywords:
      "dragon cursor, svg animation, cursor effect, creative coding, javascript animation",
  });
  ...
  useEffect(() => {
    ...
    const svg = stage.querySelector(".dragon-cursor-svg");
    const xmlns = "http://www.w3.org/2000/svg";
    const xlinkns = "http://www.w3.org/1999/xlink";

    const holder = document.createElementNS(xmlns, "svg");
    holder.innerHTML = DRAGON_DEFS;
    ...
    stage.ownerDocument.defaultView.addEventListener("resize", resize, false);
    ...
    const handlePointerMove = (e) => {
      ...
      pointer.x = Math.max(0, Math.min(width, e.clientX - rect.left));
      pointer.y = Math.max(0, Math.min(height, e.clientY - rect.top));
      rad = 0;
    };
    ...
    const run = () => {
      ...
      for (let i = 1; i < N; i++) {
        ...
        const tx = Math.max(0, Math.min(width, (ep.x + e.x) / 2));
        const ty = Math.max(0, Math.min(height, (ep.y + e.y) / 2));
        e.use.setAttributeNS(
          null,
          "transform",
          `translate(${tx},${ty}) rotate(${
            (180 / Math.PI) * a
    ...
    return () => {
      ...
      stage.ownerDocument.defaultView.removeEventListener(
        "resize",
        resize,
        false,
      );
  ...
  return (
    <div className="dragon-cursor-page" ref={stageRef}>
      <svg className="dragon-cursor-svg" />
    </div>
  );
};
