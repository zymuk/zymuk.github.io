...
1. **[INSTALLATION.md](INSTALLATION.md)** — Hướng dẫn cài đặt, chạy dev, build và triển khai production.
2. **[ARCHITECTURE.md](ARCHITECTURE.md)** — Kiến trúc hệ thống, luồng dữ liệu và chi tiết từng mô-đun.
3. **[API.md](API.md)** — Hợp đồng dữ liệu (data contract): file JSON tĩnh, khóa `localStorage` và các Web API trình duyệt.
...
Yêu cầu: **Node.js ≥ 16** và **Yarn 1.x** (hoặc npm). Chi tiết xem [INSTALLATION.md](INSTALLATION.md).
...
Tài liệu trung thực với hiện trạng code. Trong quá trình phân tích, tôi phát hiện những điểm cần lưu ý (chi tiết trong [ARCHITECTURE.md](ARCHITECTURE.md)):
