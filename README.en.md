...
| **Animations** | `/animations/dragon-cursor`, `/animations/generative-lines`, `/animations/lightbeams`, `/animations/blend-overlay`, `/animations/aizawa-attractor` | Five animation demos embedded directly into the codebase: an SVG dragon that follows the cursor (Dragon Cursor), generative line art on canvas, glowing particle text beams (Lightbeams), a background image plus a continuously shifting color overlay using CSS `mix-blend-mode` (Blend Overlay), and a holographic 3D Aizawa attractor built with Three.js (Aizawa Attractor) |
